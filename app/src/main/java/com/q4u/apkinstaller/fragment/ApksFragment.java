...
import com.q4u.apkinstaller.activity.HomeActivity;
import com.q4u.apkinstaller.adapter.ApksAdapter;
...
public class ApksFragment extends Fragment implements ApksAdapter.ApksOnClickListener, RefreshpageInterface {
    ...
    ArrayList<ApkListModel> list;
    private static final int PERMISSION_REQUEST_CODE = 100;
    String filePath;
    List<ApkListModel> apkList,sortedList;
    Dialog dialog,dialog1;
    int lastSortingType = 0;
    ArrayList<String> permissionsList;
    Toolbar toolbar;
    int position;
    ApkListModel selectedItem;
    ...
    public void setDialogApkDetails(ApkListModel apkListModel)
    {
            ...
            if (file.exists()) {
                ...
                if (fileNameArray[fileNameArray.length - 1].equals("apk")) {
                    if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.N) {
                        Uri downloaded_apk = FileProvider.getUriForFile(getActivity(), getActivity().getApplicationContext().getPackageName() + ".provider", file);
    ...
    public void deleteApk()
    {
        ...
        builder.setMessage("Do you want to delete this APK?").setPositiveButton("Ok",(dialogInterface, i) -> {
            File file = new File(selectedItem.getApkPath());
            file.delete();
            if(file.exists()){
                try {
